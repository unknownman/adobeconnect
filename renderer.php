...
//            $html .= html_writer::tag('label', $meetingdetail->url, $param);
    ...
    public function display_meeting_recording($recordings, $cmid, $groupid, $sourcescoid)
    {
        ...
        $html .= html_writer::start_tag("div", array('class'=>"limiter"));
        $html .= html_writer::start_tag("div", array('class'=>"container-table100"));
        $html .= html_writer::start_tag("div", array('class'=>"wrap-table100"));
        $html .= html_writer::start_tag("div", array('class'=>"table100"));
        $param = array('class' => 'aconrecording table');
        $html .= html_writer::start_tag('table', $param);
        $html .=  html_writer::start_tag('thead');
        $html .= html_writer::start_tag("tr");
        $html .= html_writer::tag("td", "#");
        $html .= html_writer::tag("td", "تاریخ شروع");
        $html .= html_writer::tag("td", "تاریخ پایان");
        $html .= html_writer::tag("td", "مدت زمان");
        $html .= html_writer::tag("td", "مشاهده");
        ...
        foreach ($recordings as $key => $recordinggrp) {
            if (!empty($recordinggrp)) {
                foreach ($recordinggrp as $recording_scoid => $recording) {
                    ...
                    $html .= html_writer::start_tag('tr', $param);
                    $html .= html_writer::tag('td', ++$i, array("scope"=>"row"));
                    $date = new DateTime("$recording->startdate", core_date::get_user_timezone_object());
                    $startDate = userdate($date->getTimestamp(), '📅 %a، %d %b %Y ⏰ %H:%M:%S');
                    $html .= html_writer::tag('td', html_entity_decode($startDate));
                    $date2 = new DateTime("$recording->enddate", core_date::get_user_timezone_object());
                    $endDate = userdate($date2->getTimestamp(), '📅 %a، %d %b %Y ⏰ %H:%M:%S');
                    $html .= html_writer::tag('td', html_entity_decode($endDate));
                    $durationH = intval($recording->duration /3600) ;
                    $durationH = $durationH ? $durationH : "00";
                    $durationM = intval(($recording->duration - ($durationH * 3600))/60);
                    $durationM = $durationM ? $durationM : "00";
                    $durationS = intval(($recording->duration - (($durationH * 3600) + ($durationM*60))));
                    $durationS = $durationS ? $durationS : "00";
                    $html .= html_writer::tag('td', html_entity_decode("{$durationH}:{$durationM}:{$durationS}"));
                    
                    $html .= html_writer::start_tag('td');
                    $param = array('target' => '_blank', 'class'=>'btn btn-primary  btn-block');
    ...
    /**
     * nemaiesh hozoor dar class
     * @Ali Masoudi
     */
    public function display_meetin_attendee($AttendeeReport)
    {
        $html       = '';
        // Display the meeting name field and value
        $param = array('id' => 'aconfldset2', 'class' => 'aconfldset');
        ...
        $html .= html_writer::tag('h3', get_string('میزان حضور', 'adobeconnect'), $param);
        $html .= html_writer::start_tag("div", array('class'=>"limiter"));
        $html .= html_writer::start_tag("div", array('class'=>"container-table100"));
        $html .= html_writer::start_tag("div", array('class'=>"wrap-table100"));
        $html .= html_writer::start_tag("div", array('class'=>"table100"));
        ...
        $html .= html_writer::start_tag("tr");
        $html .= html_writer::tag("td", "#");
        $html .= html_writer::tag("td", "کد کاربری");
        $html .= html_writer::tag("td", "نام");
        $html .= html_writer::tag("td", " مدت زمان حضور");
        $html .= html_writer::end_tag("tr");
        $html .=  html_writer::end_tag('thead');
        $html .=  html_writer::start_tag('tbody');
        foreach ($AttendeeReport as $key => $attendee) {
            $params = $param;
            if(strlen($attendee[0])<9) {
                $params['style'] = 'background-color: #eee';
            }
            $html .= html_writer::start_tag('tr', $params);
            $i++;
            $html .= html_writer::tag("td", $i);
            $html .= html_writer::tag("td", (string)$attendee[0]);
            $html .= html_writer::tag("td", (string)$attendee[1]);
            $html .= html_writer::tag("td", (string)$attendee[2].":00");
