...
/*
		...
		$verbose = fopen('/tmp/medga.txt','w+');
		curl_setopt($ch, CURLOPT_STDERR, $verbose);
*/
		/** Medga end */
/*
		$fh =  fopen('/tmp/medga.txt','w+');
		fwrite($fh, "request : ".$this->_xmlrequest."\n");
		fwrite($fh, "url : ". $serverurl . $querystring."\n");
		fwrite($fh, "header : ". json_encode($header)."\n");
		fwrite($fh, "agent : ". $_SERVER['HTTP_USER_AGENT']."\n");
		fclose($fh);
*/
        $result = curl_exec($ch);
        curl_close($ch);
