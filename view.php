...
if (!empty($meetscoids)) {
    ...
    foreach($meetscoids as $scoid) {
        ...
        // === || @Ali Masoud - el hacer todos attendee 
        $aconnect->create_request(['action'=>'report-meeting-attendance','sco-id'=>$scoid->meetingscoid]);
        if ($aconnect->call_success()) {
            $attendee = (array)simplexml_load_string($aconnect->_xmlresponse);
            foreach ($attendee['report-meeting-attendance']->row as $person) {
                $personLoginArray = (array)$person->login;
                $personArray = (array)$person;
                if ($personArray['date-created'] && $personArray['date-end']) {
                    $personArray['duration'] = abs(strtotime($personArray['date-created']) - strtotime($personArray['date-end']))/60;
                } else {
                    $personArray['duration'] = 0;
                }    
                $attendeeList[$personLoginArray[0]][] = $personArray;
            }

            foreach ($attendeeList as  $key=>$listItem) {
                foreach ($listItem as $perItem) {
                    $du = (float)$perItem['duration']+ $du;
                }
                $h = floor($du  / 60);
                $m = floor($du - $h*60);
                $time = str_pad(
                ...
                $AttendeeReport[$key] = array($key, $listItem[0]['participant-name'], $time);
                $du = 0;
            }
        }
...
if ($showrecordings and !empty($recordings)) {
    ...
    // Echo Hozoor dar class -- Ali Masoudi
    if (has_capability('moodle/course:manageactivities', $context, $usrobj->id, false))
    {
        echo $renderer->display_meetin_attendee($AttendeeReport);
    }
